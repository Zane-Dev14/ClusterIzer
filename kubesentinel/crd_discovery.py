...
KNOWN_CRDS = {
    "argoproj.io": {"v1alpha1": ["Application", "AppProject"]},
    "networking.istio.io": {
        "v1beta1": ["VirtualService", "DestinationRule", "Gateway", "ServiceEntry"]
    },
    "security.istio.io": {"v1beta1": ["AuthorizationPolicy", "PeerAuthentication"]},
                ...
                for kind in kinds:
                    try:
                        ...
                        if resources:
                            ...
                            if count >= MAX_CRDS:
                                logger.warning(
                                    f"CRD limit reached ({MAX_CRDS}); stopping discovery"
                                )
                                return crds, errors
                        ...
                        if e.status != 404:  # Not a "not found" error
                            error_msg = (
                                f"Failed to fetch {group}/{version}/{kind}: {e.reason}"
                            )
                            logger.debug(error_msg)
                            errors.append(error_msg)
                    except Exception as e:
                        error_msg = (
                            f"Error discovering {group}/{version}/{kind}: {str(e)}"
                        )
                        logger.debug(error_msg)
                        errors.append(error_msg)

        logger.info(
            f"CRD discovery complete: {count} resources found in {len(crds)} CRD groups"
        )
    ...
    try:
        ...
        if namespace:
            ...
            items = api.list_cluster_custom_object(
                group=group, version=version, plural=plural, limit=MAX_CRDS
            ).get("items", [])
...
def _extract_kind_specific_fields(
    ...
    # CertManager Issuer
        ...
        fields["issuer_type"] = next(
            (key for key in spec.keys() if key not in ["email", "server"]), "unknown"
        )
