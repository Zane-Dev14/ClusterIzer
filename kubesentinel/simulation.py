...
def simulate_node_failure(cluster_snapshot: Dict[str, Any], graph_summary: Dict[str, Any], node_name: str) -> Dict[str, Any]:
    """
    ...
    for pod in affected_pods:
        ...
        if owner_kind == "Deployment":
            workload = next((d for d in deployments if d.get("name") == owner_name and d.get("namespace") == pod.get("namespace")), None)
        elif owner_kind == "StatefulSet":
            workload = next((s for s in statefulsets if s.get("name") == owner_name and s.get("namespace") == pod.get("namespace")), None)
    ...
    for svc in services:
        ...
        if target_deployment:
            ...
            for workload in affected_workloads:
                if f"{workload.get('namespace')}/{workload.get('name')}" == target_deployment:
    ...
    if critical_workloads:
        summary_parts.append(f"⚠️  CRITICAL: {len(critical_workloads)} workloads would experience service outage (single replica).")
    
    if degraded_workloads:
        summary_parts.append(f"⚠️  DEGRADED: {len(degraded_workloads)} workloads would experience performance degradation.")
    ...
    return {
        "node": node_name,
        "affected_pods": [{"name": p.get("name"), "namespace": p.get("namespace")} for p in affected_pods],
        "affected_workloads": affected_workloads,
        "affected_services": affected_services,
        "impact_severity": impact_severity,
        "summary": summary,
        "recommendations": _generate_recommendations(affected_workloads, critical_workloads)
    }


def _generate_recommendations(workloads: List[Dict[str, Any]], critical: List[str]) -> List[str]:
