...
def test_cost_check_single_replica_deployments():
    ...
    state: InfraState = {
        "user_query": "cost",
        "cluster_snapshot": {"nodes": [], "pods": [], "deployments": []},
        "graph_summary": {
    ...
    assert len(findings) > 0, (
        "Expected at least one finding for single replica deployments"
    )
    single_replica_finding = next(
        (f for f in findings if "single replica" in f.get("analysis", "").lower()), None
    )
    assert single_replica_finding is not None, (
        "Expected finding about single replica deployments"
    )
...
def test_cost_check_underutilized_nodes():
    ...
    state: InfraState = {
        ...
        "graph_summary": {"single_replica_deployments": []},
        "signals": [],
        "risk_score": {},
        "planner_decision": [],
        "failure_findings": [],
    ...
    underutil_finding = next(
        (f for f in findings if "under 30%" in f.get("analysis", "").lower()), None
    )
...
def test_cost_check_hpa_candidates():
    ...
    hpa_finding = next(
        (f for f in findings if "autoscaling" in f.get("analysis", "").lower()), None
    )
...
def test_cost_check_over_requested_resources():
    ...
    over_req_finding = next(
        (
            f
            for f in findings
            if "resource" in f.get("resource", "").lower()
            and (
                "over" in f.get("analysis", "").lower()
                or "exceed" in f.get("analysis", "").lower()
            )
        ),
        None,
    )
    assert over_req_finding is not None, (
        f"Expected finding about over-requested resources, got findings: {findings}"
    )
...
def test_cost_check_no_issues():
    ...
    assert len(critical_findings) == 0, (
        f"Expected no critical/high findings, got {critical_findings}"
    )
