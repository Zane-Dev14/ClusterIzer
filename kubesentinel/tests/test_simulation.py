...
def test_simulate_node_failure_single_replica():
    ...
    cluster_snapshot = {
        ...
        "deployments": [{"name": "api", "namespace": "default", "replicas": 1}],
        "statefulsets": [],
    ...
    graph_summary = {
        "ownership_index": {
            "default/api-abc123": {"owner_kind": "Deployment", "owner_name": "api"}
        },
        "service_to_deployment": {"default/api": "default/api"},
        "deployment_to_pods": {"default/api": ["default/api-abc123"]},
    }
    ...
    assert result["impact_severity"] == "critical", (
        f"Expected critical impact, got {result['impact_severity']}"
    )
    ...
    assert (
        "Single replica" in workload["reason"] or "outage" in workload["reason"].lower()
    )
...
def test_simulate_node_failure_multi_replica():
    ...
    cluster_snapshot = {
        ...
        "deployments": [{"name": "api", "namespace": "default", "replicas": 5}],
        "statefulsets": [],
        "services": [],
    }
    ...
    graph_summary = {
        "ownership_index": {
            "default/api-pod-1": {"owner_kind": "Deployment", "owner_name": "api"}
        },
        "service_to_deployment": {},
        "deployment_to_pods": {"default/api": ["default/api-pod-1"]},
    }

    result = simulate_node_failure(cluster_snapshot, graph_summary, "node-1")

    assert result["impact_severity"] in ["medium", "low"], (
        f"Expected medium/low impact for 5 replicas, got {result['impact_severity']}"
    )
    assert len(result["affected_workloads"]) == 1
...
def test_simulate_node_failure_no_pods():
    ...
    cluster_snapshot = {
        "nodes": [{"name": "node-1", "cpu": "4000m", "memory": "8Gi"}],
        "pods": [],
        "deployments": [],
        "statefulsets": [],
        "services": [],
...
def test_simulate_node_failure_orphan_pod():
    ...
    assert workload["impact"] == "critical"
    assert (
        "orphan" in workload["reason"].lower()
        or "no controller" in workload["reason"].lower()
    )
...
def test_simulate_node_failure_statefulset():
    ...
    cluster_snapshot = {
        ...
        "deployments": [],
        "statefulsets": [{"name": "db", "namespace": "default", "replicas": 1}],
        "services": [],
    }

    graph_summary = {
        "ownership_index": {
            "default/db-0": {"owner_kind": "StatefulSet", "owner_name": "db"}
        },
        "service_to_deployment": {},
        "deployment_to_pods": {},
    }
...
def test_simulate_node_failure_multiple_workloads():
    ...
    graph_summary = {
        "ownership_index": {
            "default/api-pod": {"owner_kind": "Deployment", "owner_name": "api"},
            "default/worker-pod": {"owner_kind": "Deployment", "owner_name": "worker"},
        },
        "service_to_deployment": {},
        "deployment_to_pods": {},
    }
