...
def build_graph(state: InfraState) -> InfraState:
    ...
    ownership_index, broken_refs = _build_ownership_index(
    ...
    service_to_deployment = _map_services_to_deployments_via_labels(
        services, pods, ownership_index
    )
    ...
    single_replica_deployments = [
        dep["name"] for dep in deployments if dep["replicas"] == 1
    ]
    node_fanout_count: dict[str, int] = defaultdict(int)
    for pod in pods:
    ...
    graph_summary = {
        ...
        "pod_to_node": {
            f"{p['namespace']}/{p['name']}": p.get("node_name", "unscheduled")
            for p in pods
        },
...
def _build_ownership_index(
    ...
    for rs in replicasets:
        uid = rs.get("uid")
        if (
            uid and isinstance(uid, str) and uid.strip()
        ):  # Validate UID exists and is non-empty
            ...
            if owner.get("kind") == "Deployment":
                ...
                if owner_uid and rs.get("uid"):  # Validate both UIDs are present
                    if owner_uid in dep_by_uid:
                        rs_to_dep[f"{rs['namespace']}/{rs['name']}"] = dep_by_uid[
                            owner_uid
                        ]
                    else:
...
def _validate_ownership_index_schema(
    ownership_index: Dict[str, Dict[str, Optional[str]]],
) -> List[str]:
    """
    ...
    for pod_key, chain in ownership_index.items():
        ...
        for field, value in chain.items():
            if value is not None and not isinstance(value, str):
                errors.append(
                    f"Pod {pod_key}: field '{field}' is {type(value).__name__}, expected None or str"
                )

    return errors


def _build_crd_ownership_chains(
    ...
    for crd_group, crd_resources in crds.items():
        for resource in crd_resources:
            ...
            if owner_refs:
                ...
                top_owner = (
                    f"{resource['namespace']}/{first_owner.get('name', 'unknown')}"
                )
...
def _map_services_to_deployments_via_labels(
...
def _map_deployments_to_pods_via_ownership(
    pods: List[Dict[str, Any]], ownership_index: Dict[str, Dict[str, Optional[str]]]
) -> Dict[str, List[str]]:
    result = defaultdict(list)
